...
const toggleVideoLike = asyncHandler(async (req, res) => {
  ...
  const ifLiked = await Like.exists({
  ...
  if (!ifLiked) {
    ...
    return res.status(200).json(new ApiResponse(200, ifLiked, "Video liked"));
  } else {
    const unlike = await Like.findByIdAndDelete(ifLiked._id);
    if (!unlike) {
      throw new ApiError(400, "Could not like");
    }
    return res.status(200).json(new ApiResponse(200, ifLiked, "Video unliked"));
  }
});
...
const userLikeStatus = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, returnObject, "Current video is Liked"));
});
